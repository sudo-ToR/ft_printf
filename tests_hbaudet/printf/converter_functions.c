...
/*   Updated: 2019/12/22 21:16:44 by lnoirot          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		ft_converter_d(char *str, va_list ap)
{
	...
	if (stock.arg < 0 && stock.zero <= 0 && bool == 0)
		ret += write(1, "-", 1);
	stock.width += (stock.arg < 0 && stock.zero < 0 && stock.minus < 0 && stock.flags && stock.prec > 0) ? 1 : 0;
	//printf("%d\t%d\t%d\t%d\t%d\n", stock.prec, stock.width, stock.zero, stock.minus, stock.flags);
	while (stock.width-- > 0)
		ret += write(1, "0", 1);
	if (stock.arg != 0 || (stock.flags == 0 && stock.prec != 0))
